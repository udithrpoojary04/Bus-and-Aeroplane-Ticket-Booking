...
const AddBusPage = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.post('/vehicles', { ...formData, type: 'Bus' });
            alert(`Bus Added`);
            navigate('/dashboard');
        } catch (error) {
            ...
            <div className="card">
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '600px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <input name="name" placeholder="Bus Operator Name" value={formData.name} onChange={handleChange} required className="form-input" />
                        <input name="busNumber" placeholder="Bus Number" value={formData.busNumber} onChange={handleChange} required className="form-input" />
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    ...
                    <button type="submit" className="form-button btn-primary">Add Bus</button>
                </form>
            </div>
        </div>
    );
